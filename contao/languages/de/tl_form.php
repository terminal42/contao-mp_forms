<?php

declare(strict_types=1);
$GLOBALS['TL_LANG']['tl_form']['mp_forms_legend'] = 'Mehrseitige Formulare';

/*
 * Fields
 */
$GLOBALS['TL_LANG']['tl_form']['mp_forms_getParam'] = ['Schritt Query-Parameter', 'Sie können hier optional den verwendeten Query-Parameter für die Schritte umstellen.'];
$GLOBALS['TL_LANG']['tl_form']['mp_forms_sessionRefParam'] = ['Session-Referenz Query-Parameter', 'Um gleichzeitiges Editieren in mehreren Tabs zu erlauben, muss eine Referenz übergeben werden. Hier steuern Sie den Namen des Query-Parameters.'];
$GLOBALS['TL_LANG']['tl_form']['mp_forms_backFragment'] = ['URL Fragment für die Zurück-Schaltfläche', 'Hier können Sie ein optionales URL Fragment eingeben, welches der URL hinzugefügt wird (z.B. für Sprunglinks). Lassen Sie "#" weg.'];
$GLOBALS['TL_LANG']['tl_form']['mp_forms_nextFragment'] = ['URL Fragment für die Weiter-Schaltfläche', 'Hier können Sie ein optionales URL Fragment eingeben, welches der URL hinzugefügt wird (z.B. für Sprunglinks). Lassen Sie "#" weg.'];
